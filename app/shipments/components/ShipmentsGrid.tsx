	...
	return (
		<>
			...
			<RadioGroup
				...
				onValueChange={(val) =>
					setSelectedShipmentId(val === "all" ? "all" : val)
				}
			...
			>
				...
				<div className='flex items-center w-full mb-4 px-2'>
					...
					<Button
						...
						className={`flex items-center gap-2 ${
							allSelected ? "bg-blue-500 text-white" : ""
						}`}
				...
				{sortedShipments.map((shipment) => (
					<label
						key={shipment._id}
						className='w-full relative mb-6 cursor-pointer'
						htmlFor={`shipment-${shipment._id}`}
					...
					>
						<div className='flex-shrink-0 mt-4 m-4'>
							<RadioGroupItem
								...
								className={` absolute z-30 right-4 top-8 ${
									allSelected || selectedShipmentId === shipment._id
