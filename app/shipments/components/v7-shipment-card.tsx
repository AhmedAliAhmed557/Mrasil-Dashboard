...
export function V7ShipmentCard({ shipment }: V7ShipmentCardProps) {
	...
	// Get the router and locale for direction
	const [dir, setDir] = useState<any>("rtl");
	useEffect(() => {
		setDir(typeof window !== "undefined" ? document.dir : "rtl");
	}, []);
	console.log("dir", dir);
	// Carrier details
					...
					<div className='flex flex-col justify-between flex-1 min-w-0'>
						<div className='flex flex-col gap-3 w-full'>
							...
							<div className='flex items-center gap-6'>
								...
								<span
									className={`inline-flex items-center rounded-md px-3 py-1 text-lg font-bold ${carrierInfo.color}`}
								>
									<div className='h-14 w-14 ml-6 relative overflow-hidden rounded-full border-2 border-gray-200 bg-white flex items-center justify-center'>
										{" "}
									...
									{/* More space between image and name */}
								</span>
							</div>

							{/* Order Number */}
							{/* <div className='flex items-center gap-3'>
								<span className='font-bold text-2xl text-[#294D8B] whitespace-nowrap'>
									رقم التتبع #{shipment?._id || "غير متوفر"}
								</span>
							</div> */}

							{/* Order ID */}
							<div className='flex items-center gap-3'>
								<span className='text-lg text-[#6d6a67]'>
									رقم التتبع:{" "}
									<span className='font-medium ml-2'>
										{shipment?.trackingId || "غير متوفر"}
									</span>
								</span>
							</div>
							...
							{/* City/Address */}
							<div className='flex items-center gap-3'>
								<span className='text-lg flex gap-1 items-center text-emerald-700 font-medium'>
								...
								<svg
									...
									className={`lucide lucide-arrow-right h-4 w-4 sm:h-5 sm:w-5 text-black transform rtl:rotate-180`}
								>
									<path d='M5 12h14'></path>
									<path d='m12 5 7 7-7 7'></path>
								</svg>

								<span className='flex gap-1 items-center text-lg text-[#294D8B] font-medium'>
									<MapPin className='h-5 w-5 text-[#294d8bde]' />{" "}
							...
							<div className='flex items-center gap-3'>
								<div
									className={`inline-flex items-center gap-x-3 px-4 py-2 rounded-full text-lg font-bold border ${getStatusColor()} justify-center`}
								>
						...
						>
							{isExpanded ? (
								<Button
									variant='outline'
									size='sm'
									className='w-full v7-neu-button-sm group h-8 text-xs flex items-center justify-center gap-x-2 px-0'
								>
									<span className='sr-only sm:not-sr-only'>عرض أقل</span>
								</Button>
							) : (
								<Button
									variant='outline'
									...
									className='w-full v7-neu-button-sm group h-8 text-xs flex items-center justify-center gap-x-2 px-0'
								>
									<span className='sr-only sm:not-sr-only'>عرض المزيد</span>
								</Button>
							)}
						</Button>
					</div>
				...
				<div className='px-6 pb-6'>
					<div className='rounded-lg p-4 border-t border-gray-100'>
						<div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
							{/* Shipment Details */}
							<div className='space-y-2 bg-white rounded-xl shadow-lg p-4'>
								...
								{[
									...
									value !== null ? (
										<div
											key={label}
											className='flex gap-3 items-center text-sm py-1'
										>
