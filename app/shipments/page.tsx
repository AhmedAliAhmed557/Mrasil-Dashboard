...
export default function ShipmentsPage() {
	...
	useEffect(() => {
		const filters = [];
		if (filterStatus !== "all")
			filters.push(`الحالة: ${getStatusText(filterStatus)}`);
	...
	const availableSources = [
		{
			id: "all",
			name: "جميع المصادر",
			icon: <ShoppingCart className='h-4 w-4 ml-2' />,
		},
		{
			id: "متجر إلكتروني",
			name: "متجر إلكتروني",
			icon: <Store className='h-4 w-4 ml-2' />,
		},
	...
	const sortedShipments = [...filteredShipments].sort((a, b) => {
		if (sortBy === "newest") {
			...
			return (
				priorityOrder[(a.priority ?? "عادي") as ShipmentPriority] -
				priorityOrder[(b.priority ?? "عادي") as ShipmentPriority]
			);
		}
		return 0;
	});
	...
	const processingCount = shipments.filter(
		(s) => mapApiStatusToComponentStatus(s.shipmentstates) === "processing"
	).length;
	...
	if (isLoading) {
		return (
			<V7Layout>
				<V7Content
					title='شحناتي'
					description='إدارة ومتابعة جميع شحناتك'
				>
					...
					<div className='space-y-8 pb-20'>
						<div className='text-center py-12'>
							...
							<p className='mt-2 text-sm text-[#6d6a67]'>
								حدث خطأ أثناء تحميل الشحنات. يرجى المحاولة مرة أخرى.
							</p>
						</div>
					</div>
				</V7Content>
						...
						>
							<div className='flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6'>
								<TabsList className='v7-neu-tabs'>
									<TabsTrigger
										value='all'
										className='v7-neu-tab data-[state=active]:bg-white/50 data-[state=active]:text-red-600 data-[state=active]:border-red-600'
									>
										جميع الشحنات
									</TabsTrigger>
									<TabsTrigger
									...
									<DropdownMenu>
										...
										>
											...
											>
												...
												<DropdownMenuRadioItem value='transit'>
													جاري التوصيل
												</DropdownMenuRadioItem>
